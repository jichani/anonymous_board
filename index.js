...
app.set("view engine", "ejs"), app.set("views", "./layout");
...
app.post("/write", (req, res) => {
  connection.query("SELECT * FROM board where board_id = 1", (error, rows) => {
    ...
    if (dbCryptKey == req.body.code) {
      ...
      connection.query(
        "INSERT INTO article (board_id, title, content, reg_date) VALUES (1,?,?,?)",
      ...
      // todo 익명코드 틀렸을 때 알림 뜨게하기
      // todo id 잘못 적었을 때 잘못된 페이지입니다. 알려주기
      res.redirect("/write");
    }
  });
});
